...
class pointsController {
    async index(request:Request, response:Response){
        ...
        const serializedPoints = points.map(point => {
            return {
                ...point,
                image_url: `http://192.168.2.8:2020/uploads/${point.image}`
            }
        });
    ...
    async show(request:Request, response:Response){
        ...
        const items = knex('items').join('point_items','items.id','=','point_items_item_id').where('point_items.point_id', id).select('items.title');
